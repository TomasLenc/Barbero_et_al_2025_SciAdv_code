% Summarize RSA results for continous EEG (FFT). 
...
for i_harm_sel=1:length(harm_selections)
    ...
    fname_tbl = sprintf('response-%s_feat-%s_featStim-%s_harm-%s_concatEp-%s_snr-%s_roi-%s_avgChans-%s.csv', ...
                             response, ...
                             feat_name_resp, ...
                             feat_name_stim, ...
                             harm_str, ...
    ...
    mX_snr_all_chan = subtract_noise_bins(mX_all_chan, par.snr_bins(1), par.snr_bins(2)); 
    mX_snr = subtract_noise_bins(mX, par.snr_bins(1), par.snr_bins(2)); 



    %% topo of summed amp of all harmonics 

    cmap = viridis(256); 

    frex = harmonics * 1/par.cycle_dur; 

    amp = get_amp_summary(mX_snr_all_chan, freq, frex); 

    f = plot_topoplots(amp, chanlocs, 'cmap', cmap); 

    print([fname_save_base, '_ampSumTopo'], '-dpng', '-painters', '-r1200', f); 
