% Summarize RSA results for ERP tapping responses. 
...
lpf_cutoff = par.filt_tap_erp_cut_low; 

%% prepare output paths

response = 'tap-force-erp'; 

subdirs = fullfile('response-tap-force-erp', ...
                     sprintf('lpf-%.0fHz', lpf_cutoff)...
                     ); 
save_path = fullfile(par.summary_path, subdirs); 
mkdir(save_path); 

fname_save_base = fullfile(save_path, ...
        sprintf('response-%s_feat-erp_featStim-%s_lpf-%.0fHz', ...
                         response, ...
                         feat_name_stim, ...
                         lpf_cutoff ...
                         )); 
...
fname_tbl = sprintf('response-%s_feat-erp_featStim-%s_lpf-%.0fHz.csv', ...
                         response, ...
                         feat_name_stim, ...
                         lpf_cutoff ...
                         ); 
...
for i_sub=1:length(subjects)
    ...
    fname = fullfile(load_path_sub, ...
        sprintf('%s_feat-erp_method-pearson_rdm.mat', sub_str)); 
    ...
    fname = fullfile(load_path_sub, ...
        sprintf('%s_feat-erp_featStim-%s_method-pearson_nCateg-2_perm.mat', sub_str, feat_name_stim)); 
...
if strcmp(feat_name_stim, 'erp')
    rdm_stim = rsa_stim_erp('par', par, 'lfp_cutoff', lpf_cutoff); 
...
fname = sprintf('sub-grand_feat-erp_method-pearson_rdm.mat'); 
...
s = scatter(Y(:, 1), Y(:, 2), 100, cmap_ratios, 'filled'); 
