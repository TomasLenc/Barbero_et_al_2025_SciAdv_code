% Quantify similarity between tapping RSMs obtained from ITI ratios vs.
% FFT. 
...
response_continuous = 'tap-force'; % tap-force, tap-impulse 
...
save_path = fullfile(par.deriv_path, 'summary_comparisons', 'compare_tap_X_ratio'); 
mkdir(save_path); 
...
for i_harm_sel=1:length(harm_selections)
    ...
    subdirs = fullfile(sprintf('response-%s', response_continuous), ...
    ...
    %% load data for ratios

    subdirs = fullfile(sprintf('response-tap-onsets')); 

    fname = fullfile(par.summary_path, subdirs, ...
            sprintf('response-tap-onsets_data.mat')); 

    res_ratios = load(fname); 

    %% save fname 

    fname_save_base = fullfile(save_path, ...
            sprintf('response-%s_featStim-%s_harm-%s_concatEp-%s_snr-%s_feat-XvsRatio', ...
                             response_continuous, ...
                             feat_name_stim, ...
                             harm_str, ...
                             jsonencode(concat_ep), ...
    ...
    %% use individual RDM for tap ratios as a model of RDM for tap X
    ...
    r_X = [res_X.res.r]';
    r_ratios = [res_ratios.res.r]'; 

    z_X = r_to_z(r_X); 
    z_ratios = r_to_z(r_ratios); 
    ...
    hold on ; 
    s = scatter(repmat(1, 1, length(z_ratios)), z_ratios, 60, c, 'filled'); 
    plot(repmat([0;1],1,length(z_X)), [z_X, z_ratios]', '-', 'linew', 2, 'color', c); 
    ax.XLim = [-0.5, 1.5]; 
    ...
    [h, p, ci, stats] = ttest(z_X, z_ratios);        
    
    bf10 = bf.ttest(z_X, z_ratios);   % paired samples BF
    
    txt = text(0.5, 1.1*max([z_X; z_ratios]), ...
               sprintf('r_X=%.2f r_r_a_t=%.2f\nBF10=%.2f, \np=%.2g', ...
                    mean(r_X), mean(r_ratios), bf10, p)); 
    txt.HorizontalAlignment = 'center'; 
    ...
    r_ratios_distr = cat(1, res_ratios.res.r_all_models); 

    z_X_distr = r_to_z(r_X_distr); 
    z_ratios_distr = r_to_z(r_ratios_distr); 
    
    r = diag(corr(z_X_distr', z_ratios_distr', 'type', 'pearson')); 
    z = r_to_z(r); 
