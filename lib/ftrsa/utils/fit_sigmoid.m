function [param,stat,y_pred] = fit_sigmoid(x,y,fixed_params,initial_params,x_for_pred)
...
if isempty(x_for_pred)
    x_for_pred=min(x):(max(x)-min(x))/100:max(x);
end
y_pred = f(param(isnan(fixed_params)),x_for_pred); 
