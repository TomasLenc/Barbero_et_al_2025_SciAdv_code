function [r_obs, r_null_dist, p] = perm_rdm_corr_group(rdms, n_categ, n_perm, varargin)
...
% First, it finds the best-fitting model for each RDM separately, and takes
% the average correlation coefficient of these best models across
% participants. Then the function generates a null distribution of
...
if do_parfor
    parfor i_perm=1:n_perm
        ...
        r_null_dist(i_perm) = do_one_perm(mask_idx, rdms, models, covariate); 
    end
else
    for i_perm=1:n_perm
        if mod(i_perm, 500) == 0 && verbose
