function rsa_tap_cont(sub, par_rsa, varargin)

parser = inputParser(); 

addParameter(parser, 'response', 'tap-force');% tap-force, tap-impulse
...
fname = sprintf('%s_response-%s_concatEp-%s_snr-none_X', ...
            sub_str, response, jsonencode(par_rsa.concat_ep)); 

[header_X, data_X] = CLW_load(fullfile(load_path, fname)); 

% load complex FFT with SNR subtraction 
fname = sprintf('%s_response-%s_concatEp-%s_snr-%d-%d_X', ...
            sub_str, response, ...
            jsonencode(par_rsa.concat_ep), ...
...
fname = sprintf('%s_response-%s_concatEp-%s_snr-none_mX', ...
            sub_str, response, jsonencode(par_rsa.concat_ep)); 

[header_mX, data_mX] = CLW_load(fullfile(load_path, fname)); 

% load magniude FFT with SNR subtraction 
fname = sprintf('%s_response-%s_concatEp-%s_snr-%d-%d_mX', ...
            sub_str, response, jsonencode(par_rsa.concat_ep), ...
            par.snr_bins(1), par.snr_bins(2)); 
...
fname = sprintf('%s_feat-%s_featStim-%s_method-pearson_nCateg-2', ...
